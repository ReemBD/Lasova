...
async function addProgram(managerEmail, programName) {
  ...
  managerProgramsObjectMap[managerEmail] = managerProgramsObjectMap[managerEmail]
    ? [...managerProgramsObjectMap[managerEmail], programName]
    : [programName];
  return new Promise((resolve) => {
    fs.writeFile(path.join(__dirname, _path), JSON.stringify(managerProgramsObjectMap, null, 2), () => {
      logger.info(`Added new program ${programName} managed by usre ${managerEmail}`);
